...
//#endif
    ...
    0x3068,                //    82:    Command set supported. If words 82 and 83 =0000h or // Fabio: modify to 0x3069 to enable SMART support
    ...
    0x3060,                //    85:    Command set/feature enabled. If words 85, 86, and 87 // Fabio: modify to 3061 to enable SMART support
...
{
    uart_print("ata_identify_device lba="); uart_print_int(lba); uart_print(", sector_count="); uart_print_int(sector_count); uart_print("\r\n");
    UINT16* const addr = ht_identify_data;
    UINT32 capacity;

    set_string_data(&addr[10], "0123456789", 10);
    set_string_data(&addr[23], "1234", 4);
    ...
    UINT8* bytePtr = (UINT8*) addr;

    UINT8 counter=0;
    for (UINT32 i=0; i<255*2; ++i) {
       counter+=bytePtr[i];
    }
    counter += 0xA5;
    counter = (~counter) + 1;
    UINT16 integrityWord = ((UINT16) counter) << 8;
    integrityWord |= 0xA5;
    addr[255] = integrityWord;
