...
{
    uart_print_level_1("Warning: ATA not supported command!\r\n");
	send_status_to_host(B_ABRT);
}
...
void pio_sector_transfer(UINT32 const dram_addr, UINT32 const protocol)
{
	...
	while ((GETREG(SATA_INT_STAT) & (OPERATION_OK | OPERATION_ERR)) == 0) {
        count++;
        if (count > 100000) {
            uart_print_level_1("Warning1 in pio_sector_transfer\r\n");
            count=0;
        }
    }
	...
	if (PIO_H2D == protocol)
	{
		...
		while (GETREG(SATA_FIFO_1_STATUS) & 0x007F0000) {
            count++;
            if (count > 100000) {
                uart_print_level_1("Warning2 in pio_sector_transfer\r\n");
                count=0;
            }
        }
	}
