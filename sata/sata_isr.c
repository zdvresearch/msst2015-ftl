...
SEND_RETRY:
    count++;
    if (count>100000) {
        uart_print_level_1("Warning1 in send_primitive_R_XX\r\n");
        count=0;
    }
	...
	while ((GETREG(SATA_INT_STAT) & OPERATION_OK) == 0)	// OPERATION_OK flag will be set upon the completion of R_OK or R_ERR transmission.
        count++;
        if (count>100000) {
            uart_print_level_1("Warning2 in send_primitive_R_XX\r\n");
            count=0;
        }
	{
		if (GET_TIMER_VALUE(TIMER_CH4) == 0)
		{
			...
			while (GETREG(SATA_FIFO_1_STATUS) & 0x007F0000) {	// Wait until the command layer FIFO becomes empty
                count2++;
                if (count2 > 100000) {
                    uart_print_level_1("Warning3 in send_primitive_R_XX\r\n");
                    count2=0;
                }
            }
...
static __inline void handle_srst(void)
{
	...
	else if (g_sata_context.srst)
	{
		...
		while (GETREG(SATA_FIFO_1_STATUS) & 0x007F0000) {
            count++;
            if (count > 100000) {
                uart_print_level_1("Warning in handle_srst\r\n");
                count=0;
            }
        }
...
static __inline void handle_got_cfis(void)
{
	...
	if (lba + sector_count > MAX_LBA + 1 && (cmd_type & ATR_NO_SECT) == 0) {
        ...
        if (cmd_type & (CCL_FTL_H2D | CCL_FTL_D2H)) {
            ...
            if (cmd_code == ATA_DATA_SET_MANAGEMENT) { // TRIM
                //uart_print_level_1("SATA ISR: TRIM lba="); uart_print_level_1_int(lba);
                //uart_print_level_1(" count="); uart_print_level_1_int(sector_count); uart_print_level_1("\r\n");
                SETREG(SATA_LBA, 0x3FFFFFFF);
                SETREG(SATA_SECT_CNT, sector_count);
                SETREG(SATA_INSERT_EQ_W, 1);	// The contents of SATA_LBA and SATA_SECT_CNT are inserted into the event queue as a write command.
                SETREG(SATA_XFER_BYTES, sector_count * BYTES_PER_SECTOR);
                SETREG(SATA_SECT_OFFSET, 0); // Fabio: Maybe in this way the ranges list will start at the beginning of the buffer
